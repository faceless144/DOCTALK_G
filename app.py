...
def index_pdf(pdf_path,temp_dir):
    try:
        ...
        with st.spinner("Indexing documents..."):
            ...
            service_context = ServiceContext.from_defaults(llm=Groq(model="llama3-70b-8192", api_key=Groq.api_key, temperature=0.2, system_prompt="You are assistant researcher who is a famous researcher to evaluate scientific articles. It is extremely important research. Before responding verify the context very carefully. Your response should be very clear and specific, wherever possible quote references from the context. Add relavent information to the response from the context. If response requires it give nicely formatted bullet points. If the questioned cannot be answered with the information within the context provided, then reply that you could not find the relavent information in the context, do not hallucinate. Be very helpful" ))
